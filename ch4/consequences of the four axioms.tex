...
\]
of tangent bundles is a bundle map\index{bundle map}. Applying \ref{axi:04.02}, one obtains the identity
\[
...
\]
Now the Whitney product theorem\index{Whitney product theorem} \ref{axi:04.03} implies that $\sw(\tau)=\sw(\tau \oplus \trivialbundle^{1})$ is equal to
\[
